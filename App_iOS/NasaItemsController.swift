...
class NasaItemsController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getItems() {
        ...
        AF.request(url, method: .get, parameters: parameters, encoding: encoding, headers: headers).responseJSON { (response) in
            print(response.result)
            self.indicator.isHidden = true
            if let items = JSON(response.value as Any)["collection"]["items"].array {
                for item in items {
