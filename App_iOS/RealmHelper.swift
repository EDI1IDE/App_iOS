...
extension Realm {
    fileprivate static let KEY_REALM_SCHEMA_VERSION = "iOS_Realm_Schema_Version"
            ...
            for item in items {
                ...
                if let realm = item.realm {
                    try realm.write {
                        ...
                        realm.create(type(of: item), value: item, update: .all)
                    }
                } else {
                    instance.beginWrite()
                    instance.create(type(of: item), value: item, update: .all)
